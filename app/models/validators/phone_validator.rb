class PhoneValidator < ActiveModel::Validator
	...
	def validate(record)
		...
		record.errors.add(:phones, I18n.t(:blank, scope: phone_scope)) unless record.phones.present? && record.phones.any?
		record.errors.add(:phones, I18n.t(:taken, scope: phone_scope)) if record.phones.present? && duplicated_phone?(record.phones.last, record.phones)
	end
